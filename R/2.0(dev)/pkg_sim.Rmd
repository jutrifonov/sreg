...
design.classifier <- function(data, S) {
  ...
  if (length(unique_sizes) == 1) {
    # All strata are the same size — no mixed design
    return(data)
  }
...
data_s <- sreg.rgen(n = 6000, tau.vec = c(0.2), n.strata = 4, cluster = FALSE, small.strata = TRUE, treat.sizes = c(1, 1), k = 2)
Y <- data$Y
S <- data$S

data_b <- sreg.rgen(n = 51, tau.vec = c(0.2), n.strata = 2, cluster = FALSE, small.strata = FALSE, treat.sizes = c(1, 1), k = 2)
...
## Simulations for the combined estimator
```{r}
nsim = 1000
tau.vec = c(0.2, 0.8)
