...
theta_hat_mult <- function(Y, D, X, S)
{
    ...
    summarise(
        ...
        X_treated = as.list(colMeans(X[D == d, , drop = FALSE], na.rm = TRUE)),
        X_control = as.list(colMeans(X[D == 0, , drop = FALSE], na.rm = TRUE)),
  ...
  mutate(
        ...
        X_diff = as.list((1/l) * (as.data.frame(X_treated)) - (1/q) * (as.data.frame(X_control))),
        k = k,
        l = l, 
        q = q
  )
...
#summary(lm_model)
    beta_hat <- coef(lm_model)[-1]

X_bar <- colMeans(as.data.frame(data_full$X))
X_dem <- sweep(as.matrix(data_full$X), 2, X_bar)


# adjusted estimator:
theta_hat_adj <- sum((data_full$Y * (data_full$D == d))) / sum((data_full$D == d)) -
                    sum((data_full$Y) * (data_full$D == 0)) / sum(data_full$D == 0) -
                    as.numeric((colMeans(X_dem[data_full$D == d, , drop = FALSE]) - 
...
#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%#%
var_hat_mult_cl <- function(Y, D, X, S, G.id, Ng, tau.hat) {
...
#-------------------------------------------------------------------
{
 ...
 data.short <- data.frame(
  ...
  data.short <- data.frame(
    A, S = block_id, data.short_sorted
  )
  #print(data.short)
  #print(block_id)
  #print(A)
