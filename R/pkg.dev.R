...
#' @param X a data frame with columns representing the cluster-level covariate values for every observation; if \code{NULL} then the estimator without linear adjustments is applied. (Note: sreg cannot use individual-level covariates for covariate adjustment in cluster-randomized experiments. Any individual-level covariates will been aggregated to their cluster-level averages)
