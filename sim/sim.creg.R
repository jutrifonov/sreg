...
sim.func <- function(sim.id)
{
  ...
  result <- tryCatch({sreg(Y, S, D, G.id, Ng, X=X, Ng.cov = T, HC1 = FALSE)}, error = function(e) { # tryCatch to avoid errors that stop the execution
...
save(simres, file = "/Users/trifonovjuri/Desktop/sreg.source/mc.files/res/creg/adj/new version/3000.RData")
###################
# Close the cluster
stopCluster(cl) 
# Close the cluster
